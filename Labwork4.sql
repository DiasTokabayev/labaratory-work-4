...
SELECT UPPER(first_name || ' ' || last_name) AS upper_name, LENGTH(last_name) AS last_name_length, SUBSTRING(email FROM 1 FOR 3) AS email_prefix
FROM employees;

SELECT first_name || ' ' || last_name AS employee_name, salary * 12 AS annual_salary, ROUND(salary / 12, 2) AS monthly_salary, salary * 0.10 AS raise_amount
FROM employees;
...
SELECT e.first_name || ' ' || e.last_name AS employee_name, e.department, AVG(a.hours_worked) AS avg_hours, e.salary
FROM employees e
...
GREATEST(MAX(salary), MIN(salary)) AS greatest_salary_value,
LEAST(MAX(salary), MIN(salary)) AS least_salary_value
FROM employees
GROUP BY department;
